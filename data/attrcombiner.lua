...
--Maxpower for L5 raised to 1000. Coal and elec cost calcs modified; many L4 and L5 combos will see a cost change, power is cheaper!
...
-- { attribute name, zero ok, min, max, rank list, ui attribute name, game->ui scale factor }
	...
	{ "speed_max", 1, 14, nil, {0.0, 20.0, 30.0, 40.0}, "landspeed", 1 },
	{ "waterspeed_max", 1, 12, nil, {0.0, 10.0, 20.0, 30.0}, "waterspeed", 1 },
	{ "airspeed_max", 1, 16, nil, {0.0, 10.0, 20.0, 30.0}, "airspeed", 1 },
	{ "powerdisplay", nil, nil, nil, {0.0, 25.0, 35.0, 45.0}, "sightradius", 1 },
...
AbilityData =
{
	...
	{ ABT_Ability, 	"soiled_land", 		3, 	0, 	0, 	0 },	--special
...
if (getgameattribute("herding") == 1) and (getgameattribute("pack_hunter") == 0) then
	...
	CostRenew = CostRenew + (((hitpoints/(1-herdarmour)) - (hitpoints/(1-armour)))/4);
end
...
CostGather = (CostGather + speedCost)*((power*1.3/max_power)^0.8)*RangeCostMult*1.1;
...
if getgameattribute("soiled_land") == 1 then
	CostRenew = CostRenew + 30 + (CreatureRank * 20);
end
